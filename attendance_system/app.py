...
def all_records():
    ...
    with get_db_connection() as conn:
        ...
        # 各レコードに対していいね済みかどうかを確認し、liked情報を追加する
        liked_record_ids = [row['record_id'] for row in conn.execute(
            'SELECT record_id FROM likes WHERE user_id = ?', (session['user_id'],)).fetchall()]
        ...
        for record in records:
            formatted_records.append({
                ...
                'formatted_date': datetime.fromisoformat(record['formatted_time']).strftime('%Y-%m-%d'),
                'formatted_time': datetime.fromisoformat(record['formatted_time']).strftime('%H:%M:%S'),
