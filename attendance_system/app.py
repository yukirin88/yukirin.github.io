...
with get_db_connection() as conn:
    try:
        conn.execute('ALTER TABLE records ADD COLUMN likes_count INTEGER DEFAULT 0')
        conn.commit()
    ...
    admin_user = conn.execute('SELECT * FROM users WHERE username = ?', ('ad',)).fetchone()
    if not admin_user:
...
@app.route('/like/<int:record_id>/<string:from_page>', methods=['POST'])
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5000, ssl_context=('mycert.pem', 'key.pem'), debug=True)
