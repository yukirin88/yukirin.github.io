...
def get_db_connection():
    ...
    conn.execute("PRAGMA journal_mode = WAL")  # パフォーマンス向上
    return conn
    ...
    with get_db_connection() as conn:
        try:
            records = conn.execute('''
                SELECT id, action, 
                    strftime('%H:%M:%S', 
                    datetime(timestamp, '+9 hours')) as formatted_time,
                    memo, likes_count 
                FROM records 
...
def calendar_view():
    ...
    return render_template(
        ...
        next_year=next_year,
        today=today,
        convert_to_jst=lambda d: datetime(year, month, d).astimezone(pytz.timezone('Asia/Tokyo')).strftime('%Y-%m-%d')
    )
